...
def main(image_ref, image_reg_ref, image_targ, aoi, allow_reg=False, view_radcal_fits=True,
    ...
    with fiona.open(aoi) as src:    # Get AOI crs -LEVI
        aoi_crs = src.crs['init']       
    ...
    #aoi_crs = rasterio.crs.CRS(aoi_meta['crs']) ########################################### Get string literal of CRS for shapefile
    print('target crs~~~~~~ : ', target_crs, '\n aoi crs~~~~~~~', aoi_crs)
    print('reg_ref_crs.to_epsg() : ', reg_ref_crs.to_epsg(), type(reg_ref_crs.to_epsg()), '\naoi_crs : ', aoi_crs, type(aoi_crs))
    # TODO this may fail if a crs is not already set up as an epsg
    
    ########### LEVI updating aoi shapefile projection #########
    aoi_pro = gpd.read_file(aoi)       # open ifle
    aoi_pro = aoi_pro.to_crs(epsg=reg_ref_crs.to_epsg())  # update projection
    print('updated aoi crs: ', aoi_pro.crs)
    aoi = os.path.join(outdir, 'aoi_reproj.shp')
    aoi_pro.to_file(aoi)   #write new shapefile to outdir
    #aoi = os.path.abspath(aoi_pro)  # point aoi variable to updated shapefile
    
    #############
    if not rad_ref_crs.to_epsg() == reg_ref_crs.to_epsg() == target_crs.to_epsg(): # == int(aoi_crs):
        ...
        '''
        if rad_ref_crs.to_string() != aoi_crs:
            if outdir:
                dir_target = outdir
            else:
                dir_target = os.path.split(image_targ)[0]
            reproj_aoi = os.path.join(dir_target, os.path.split(aoi)[1][:-4] + '_reproj.shp')
            reproj_aoi = aoi.to_crs(epsg = reg_ref_crs)
            #update_projection(image_ref, aoi, reprcoj_aoi, outdir=dir_target)
            reproj_aoi
            print(aoi)
        '''
    # LEVI ADDITION
    # update aoi projection to match targ.
    
    # crop target image to aoi.
    image_targ = clip_to_shapefile(image_targ, aoi, outname='aoi_clipped_raster.tif')
...
if __name__ == '__main__':
    ...
    #image1 = r"D:\Sync\Planet_Downloads\Sentinel\AFRF_2017\S2A_MSIL1C_20170804T190921_N0205_R056_T10UED_20170804T191650\BGRN\20170804_stack.tif"
    assert isinstance(image1, str)
    image_reg_ref = input("Location of image with desired georeferencing: ")
    #image_reg_ref = r"D:\Sync\Planet_Downloads\Sentinel\AFRF_2017\S2A_MSIL1C_20170804T190921_N0205_R056_T10UED_20170804T191650\BGRN\20170804_stack.tif"
    assert isinstance(image_reg_ref, str)
    image2 = input("Location of target image to be radiometrically normalized: ")
    #image2 = r'‪D:\Sync\tsn_dat\20170822_check1.tif'
    assert isinstance(image_reg_ref, str)
    ...
    if view_radcal_fits == "y":
        ...
        print("Must choose y or n. Try again.")
        quit()
    aoi = input('Trim to AOI? filepath to .shp file or ''n''')
    assert isinstance(aoi, str)
    if aoi == 'n':
        aoi = False
    main(image1, image_reg_ref, image2, udm=udm, allow_reg=allowRegistration,
         view_radcal_fits=view_radcal_fits, outdir=output_dir, aoi=aoi)
